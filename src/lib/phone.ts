/** ITU-T dialing codes for signup phone capture. */
...
export function findPhoneCountry(isoOrDial: string) {
  const raw = String(isoOrDial || "").trim();
  if (!raw) return PHONE_COUNTRIES.find((c) => c.iso === DEFAULT_PHONE_COUNTRY_ISO)!;
  const byIso = PHONE_COUNTRIES.find((c) => c.iso === raw.toUpperCase());
  if (byIso) return byIso;
  const dial = raw.startsWith("+") ? raw : `+${raw.replace(/\D/g, "")}`;
  return (
...
export function normalizePhoneInput(
  countryIso: string,
  nationalNumber: string,
): { ok: true; phone: NormalizedPhone } | { ok: false; error: string } {
  ...
  if (national.length < 6 || national.length > 15) {
    return { ok: false, error: "Enter a valid phone number (6–15 digits)." };
  }
  const dialDigits = digitsOnly(country.dial);
  if (national.startsWith(dialDigits)) {
    return { ok: false, error: "Enter the number without the country code." };
  }
  return {
    ok: true,
    phone: {
...
export function formatPhoneDisplay(phone?: {
  ...
  if (!phone) return "";
  if (phone.phone) return String(phone.phone);
  const code = String(phone.phoneCountryCode || "").trim();
  const national = String(phone.phoneNational || "").trim();
  if (code && national) return `${code}${national.startsWith("0") ? national.slice(1) : national}`;
