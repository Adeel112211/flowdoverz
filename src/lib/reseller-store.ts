...
import {
  DEFAULT_TRIAL_SEAT_HOURS,
  normalizeTrialSeatHours,
  resellerClientTrialExpiryFromNow,
  trialSeatHoursLabel,
} from "@/lib/reseller-trial";
...
export type ResellerRecord = {
  ...
  /** Free trial seats granted by admin (separate from paid seats). */
  trialSeatsGranted: number;
  /** When true, reseller can register clients on trial seats. */
  trialSeatsEnabled: boolean;
  /** Hours each trial seat lasts after registration (default 5). */
...
export { DEFAULT_TRIAL_SEAT_HOURS, trialSeatHoursLabel } from "@/lib/reseller-trial";
...
export function publicResellerPlanOptions(record: ResellerRecord) {
  const paid = record.allowedSeatPlans.map((plan) => ({
    id: plan,
  ...
  return [
    {
      id: "trial" as const,
      label: `${trialSeatHoursLabel(hours)} trial`,
      pricePerSeatPkr: 0,
    },
    ...paid,
  ];
...
const TRIAL_PLAN_ALIASES = new Set(["trial", "trail", "free_trial", "free-trial", "freetrial"]);

function normalizeResellerPlanToken(raw: unknown) {
  return String(raw ?? "")
    .trim()
    .toLowerCase()
    .replace(/[\s_-]+/g, "");
...
function planTokenMeansTrial(raw: unknown) {
  const token = normalizeResellerPlanToken(raw);
  if (!token) return false;
  if (TRIAL_PLAN_ALIASES.has(token)) return true;
  return token.includes("trial") || token.includes("trail");
}

/** True when any known request field indicates a trial seat. */
export function resellerClientPlanLooksLikeTrial(input: Record<string, unknown>) {
  const keys = ["subscriptionPlan", "plan", "seatPlan", "seat", "type", "mode", "seatType"];
  for (const key of keys) {
    if (planTokenMeansTrial(input[key])) return true;
  }
...
async function withResellerRegistrationLock<T>(email: string, fn: () => Promise<T>): Promise<T> {
  ...
  const ref = db.collection("_locks").doc(`reseller_reg_${key}`);
  const owner = randomBytes(8).toString("hex");

  for (let attempt = 0; attempt < 12; attempt += 1) {
    const acquired = await db.runTransaction(async (tx) => {
      const snap = await tx.get(ref);
      const until = Date.parse(String(snap.data()?.until || "")) || 0;
      if (snap.exists && until > Date.now() && snap.data()?.owner !== owner) {
        return false;
      }
      tx.set(ref, { owner, until: new Date(Date.now() + 90_000).toISOString() });
      return true;
    });
    if (acquired) {
      try {
...
/** @deprecated use DEFAULT_TRIAL_SEAT_HOURS from reseller-trial */
export const RESELLER_CLIENT_TRIAL_MS = DEFAULT_TRIAL_SEAT_HOURS * 60 * 60 * 1000;

export { resellerClientTrialExpiryFromNow };
...
> {
  ...
  try {
    return await withResellerRegistrationLock(normalizedEmail, async () => {
      if (await getUserRecord(normalizedEmail)) {
      ...
      if (wantsTrial) {
        ...
        if (trialLeft <= 0) {
          return {
            ok: false,
            error: "No trial seats left. Ask the owner to add more trial seats.",
            status: 403,
          };
        }
    ...
    if (error instanceof Error && error.message === "REGISTRATION_BUSY") {
      return {
        ok: false,
        error: "Registration is already in progress for this email. Wait a moment and try again.",
        status: 409,
      };
    }
    throw error;
...
export async function createReseller(input: ResellerInput): Promise<{
  ...
  const record: ResellerRecord = {
    ...
    trialSeatsGranted: Math.max(0, Math.floor(Number(input.trialSeatsGranted) || 0)),
    trialSeatsEnabled: Boolean(input.trialSeatsEnabled),
    trialSeatHours: normalizeTrialSeatHours(input.trialSeatHours),
  ...
  const next: ResellerRecord = {
    ...
    trialSeatHours:
      input.trialSeatHours !== undefined
        ...
        : current.trialSeatHours || DEFAULT_TRIAL_SEAT_HOURS,
    seatDays:
...
export async function addResellerTrialSeats(
  id: string,
  seats: number,
  meta?: { note?: string; hours?: number },
): Promise<PublicReseller> {
  const count = Math.floor(Number(seats));
  if (!Number.isFinite(count) || count < 1) {
    throw new Error("Enter how many trial seats to grant.");
  }
  const current = await getReseller(id);
  if (!current) throw new Error("Reseller not found.");
  const now = new Date().toISOString();
  const trialSeatsGranted = (current.trialSeatsGranted || 0) + count;
  const trialSeatHours =
    meta?.hours !== undefined
      ? normalizeTrialSeatHours(meta.hours)
      : current.trialSeatHours || DEFAULT_TRIAL_SEAT_HOURS;
  const db = requireDb();
  await db.collection(COLLECTION).doc(id).set(
    {
      trialSeatsGranted,
      trialSeatsEnabled: true,
      trialSeatHours,
      updatedAt: now,
    },
    { merge: true },
